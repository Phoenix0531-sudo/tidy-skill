...
These files are examples of what the Agent Tidy Skill aims to prevent or manage.
...
| `implementation_plan.md` | Should have been `docs/architecture.md` if it had lasting value, or kept in chat if temporary. |
| `cleanup_report.md` | Meta-litter — a report about cleaning up. The cleanup script already logs its actions. |
...
1. **Generic filename** — `plan.md`, `report.md`, `summary.md` tell you nothing about what they contain.
2. **No context** — No date, no task name, no project reference in the filename or header.
...
7. **Redundant** — Content duplicates the chat conversation, a commit message, or a PR description.
...
|---|---|
| `./plan.md` | Keep the plan in the chat, or put it in `.agent_tmp/plan_<task>_<date>.md` |
| `./todo.md` | Use the chat or the project's issue tracker; delete at task end |
...
| `./implementation_plan.md` | If temporary → `.agent_tmp/`. If permanent → `docs/`. |
...
| D — Self-congratulatory | `./lessons.md` | If lessons exist, put them in a team wiki or retro doc |
...
| C → A (mislabeled) | `./migration_plan.md` | If it has lasting value, move to `docs/` |
